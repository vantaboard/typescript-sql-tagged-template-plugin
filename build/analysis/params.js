...
const getColumn = (columnRef, relations, warnings) => {
    ...
    if (columnRef.ColumnRef.fields.length > 2) {
        warnings.push((0, utils_1.other)("ColumnRef has more then 2 fields", columnRef));
    }
    const getField = (field) => {
        if ((0, pg_query_emscripten_type_guards_1.isPgString)(field)) {
...
const getRelations = (node) => {
    ...
    if ((0, pg_query_emscripten_type_guards_1.isPgJoinExpr)(node)) {
        (0, utils_1.assignMap)(relations, getRelations(node.JoinExpr.larg), getRelations(node.JoinExpr.rarg));
    }
    else if ((0, pg_query_emscripten_type_guards_1.isPgRangeVar)(node)) {
...
const getRelationsForFromClause = (fromClause) => {
    const relations = new Map();
    (0, utils_1.assignMap)(relations, ...fromClause.map(getRelations));
    return relations;
};
const getOperator = (expr) => {
    const name = expr.A_Expr.name;
    if (name && (0, pg_query_emscripten_type_guards_1.isPgNodeArray)(name)) {
        const first = name[0];
        if (first && (0, pg_query_emscripten_type_guards_1.isPgString)(first)) {
...
const getParamMapForWhereClause = (whereClause, relations, warnings) => {
    const params = [];
    if ((0, pg_query_emscripten_type_guards_1.isPgA_Expr)(whereClause)) {
        const expr = whereClause.A_Expr;
        switch (expr.kind) {
            case 7 /* PgA_Expr_Kind.AEXPR_IN */:
                if ((0, pg_query_emscripten_type_guards_1.isPgNodeArray)(expr.rexpr)) {
                    if ((0, pg_query_emscripten_type_guards_1.isPgColumnRef)(expr.lexpr)) {
                        const column = getColumn(expr.lexpr, relations, warnings);
                        for (const field of expr.rexpr) {
                            if ((0, pg_query_emscripten_type_guards_1.isPgParamRef)(field)) {
                                params.push({
                        ...
                        warnings.push((0, utils_1.notSupported)("where clause", whereClause));
                    }
                }
                else {
                    warnings.push((0, utils_1.notSupported)("where clause", whereClause));
                ...
                break;
            case 0 /* PgA_Expr_Kind.AEXPR_OP */:
            case 1 /* PgA_Expr_Kind.AEXPR_OP_ANY */:
            case 2 /* PgA_Expr_Kind.AEXPR_OP_ALL */:
                const operator = getOperator(whereClause);
                if (!operator || !COMPARISON_OPERATORS.includes(operator)) {
                    warnings.push((0, utils_1.notSupported)("where clause", whereClause));
                }
                else if ((0, pg_query_emscripten_type_guards_1.isPgParamRef)(expr.rexpr)) {
                    const isArray = [
                        1 /* PgA_Expr_Kind.AEXPR_OP_ANY */,
                        2 /* PgA_Expr_Kind.AEXPR_OP_ALL */,
                    ].includes(expr.kind);
                    if ((0, pg_query_emscripten_type_guards_1.isPgColumnRef)(expr.lexpr)) {
                        params.push({
                    ...
                    else if ((0, pg_query_emscripten_type_guards_1.isPgA_Expr)(expr.lexpr) &&
                        JSON_OPERATORS.includes(getOperator(expr.lexpr) || "") &&
                        (0, pg_query_emscripten_type_guards_1.isPgColumnRef)(expr.lexpr.A_Expr.lexpr) &&
                        (0, pg_query_emscripten_type_guards_1.isPgA_Const)(expr.lexpr.A_Expr.rexpr)) {
                        ...
                        params.push({
                            ...
                            usedWith: {
                                ...
                                jsonPath: {
                                    path: (0, pg_query_emscripten_type_guards_1.isPgInteger)(pathVal)
                                        ? pathVal.Integer.ival
                                        : (0, pg_query_emscripten_type_guards_1.isPgString)(pathVal)
                                            ? pathVal.String.str
                                            : "<UNKNOWN>",
                ...
                else if ((0, pg_query_emscripten_type_guards_1.isPgSubLink)(expr.rexpr) &&
                    expr.rexpr.SubLink.subselect &&
                    (0, pg_query_emscripten_type_guards_1.isPgSelectStmt)(expr.rexpr.SubLink.subselect)) {
                    params.push(...(0, exports.getParamMapForSelect)(expr.rexpr.SubLink.subselect, warnings, relations));
                }
                else if (!(0, pg_query_emscripten_type_guards_1.isPgColumnRef)(expr.rexpr) &&
                    !(0, pg_query_emscripten_type_guards_1.isPgA_Const)(expr.rexpr)) {
                    warnings.push((0, utils_1.notSupported)("where clause", whereClause));
                }
                break;
            default:
    ...
    else if ((0, pg_query_emscripten_type_guards_1.isPgBoolExpr)(whereClause)) {
        const expr = whereClause.BoolExpr;
        for (const arg of expr.args) {
            params.push(...getParamMapForWhereClause(arg, relations, warnings));
        }
    }
    else if (!(0, pg_query_emscripten_type_guards_1.isPgNullTest)(whereClause)) {
        warnings.push((0, utils_1.notSupported)("where clause", whereClause));
    }
    return params;
};
const getParamMapForUpdate = (stmt, warnings) => {
    ...
    for (const target of stmt.UpdateStmt.targetList) {
        if ((0, pg_query_emscripten_type_guards_1.isPgResTarget)(target)) {
            if ((0, pg_query_emscripten_type_guards_1.isPgParamRef)(target.ResTarget.val)) {
                params.push({
            ...
            warnings.push((0, utils_1.other)("Target is not a ResTarget", target));
        }
    }
    if (stmt.UpdateStmt.whereClause) {
        const relations = getRelations(stmt.UpdateStmt.relation);
        if (stmt.UpdateStmt.fromClause) {
            (0, utils_1.assignMap)(relations, getRelationsForFromClause(stmt.UpdateStmt.fromClause));
        }
...
const getParamMapForInsert = (stmt, warnings) => {
    ...
    if ((0, pg_query_emscripten_type_guards_1.isPgSelectStmt)(stmt.InsertStmt.selectStmt)) {
        ...
        if (select.valuesLists && stmt.InsertStmt.cols) {
            for (const valueList of select.valuesLists) {
                for (let i = 0; i < valueList.length; i++) {
                    ...
                    if ((0, pg_query_emscripten_type_guards_1.isPgParamRef)(value)) {
                        const column = stmt.InsertStmt.cols[i];
                        if ((0, pg_query_emscripten_type_guards_1.isPgResTarget)(column)) {
                            params.push({
                            ...
                            warnings.push((0, utils_1.notSupported)("colum type in select clause", column));
                        }
                    }
                }
            }
            ...
            warnings.push((0, utils_1.notSupported)("select clause", select));
        }
    }
    return params;
};
...
const getParamMapForSelect = (stmt, warnings, parentRelations) => {
    const params = [];
    const relations = new Map();
    (0, utils_1.assignMap)(relations, parentRelations, stmt.SelectStmt.fromClause &&
